...
unsigned long long bstrtoull(struct bstr str, struct bstr *rest, int base)
{
    str = bstr_lstrip(str);
    char buf[51];
    int len = FFMIN(str.len, 50);
    ...
    buf[len] = 0;
    char *endptr;
    unsigned long long r = strtoull(buf, &endptr, base);
    if (rest)
        *rest = bstr_cut(str, endptr - buf);
    return r;
}
...
void bstr_append(void *talloc_ctx, struct bstr *s, struct bstr s2)
{
    if (!s2.len)
        return;
    s->start = talloc_realloc(talloc_ctx, s->start, char, s->len + s2.len);
    memcpy(s->start + s->len, s2.start, s2.len);
    s->len += s2.len;
}
